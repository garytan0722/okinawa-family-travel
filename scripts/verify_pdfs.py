...
for variant, path in PDFS.items():
    ...
    assert "台灣虎航 IT230" in text, f"{path.name}: missing confirmed September 30 airline"
    assert "星宇航空 JX871" in text, f"{path.name}: missing corrected October 4 flight"
    assert "長榮航空 BR185" in text, f"{path.name}: missing companion October 4 flight"
    assert "譚家四口" in text and "小倆口" in text, f"{path.name}: missing corrected group labels"
    assert "曾蘿情侶" not in text and "曾羅佳" not in text, f"{path.name}: contains superseded traveler label"
    ...
    assert "15:50  →  16:25" in text, f"{path.name}: missing corrected JX871 arrival time"
    assert "20:20  →  20:55" in text, f"{path.name}: missing confirmed BR185 arrival time"
    assert "抵達時間未提供" not in text, f"{path.name}: contains stale unknown-arrival disclosure"
    assert "小倆口 BR185 起飛" in text, f"{path.name}: missing companion final-day departure"
