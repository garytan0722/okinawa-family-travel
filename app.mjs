import { getVariantDay, mapUrl, nextFixedEvent } from './src/trip-domain.mjs?v=29';
import { centeredScrollLeft, renderChecklistView, renderDay, renderEmergencyView, renderFixedEventList, renderFlightSummary, renderPrivateBookingVault, renderRainyDayView, renderSources, renderStayAction, renderTicketPassGuide, renderVariantTabs, escapeHtml } from './src/render.mjs?v=29';
import { parseOtsReservations } from './src/private-bookings.mjs?v=29';
import { clearRainBackup, selectRainBackup, toggleRainMode } from './src/rain-state.mjs?v=29';
import { createEmptyState, exportBackup, importBackup, loadState, saveState } from './src/storage.mjs?v=29';
import { installPwaUpdate } from './src/pwa-update.mjs?v=29';
...
function renderLogistics() {
  ...
  app.innerHTML = `<section class="tool-view"><p class="eyebrow">FIXED LOGISTICS</p><h1>固定行程與住宿</h1><p class="lede">這些是不能移動的時間；其餘景點都應讓位給它們。</p>${renderFlightSummary(trip)}${renderTicketPassGuide(trip, state.selectedVariant)}<div class="tool-grid">${stays}</div>${renderPrivateBookingVault(state.privateBookings, state.privateStayLocation)}${renderFixedEventList(trip.fixedEvents)}</section>`;
}
...
async function boot() {
  try {
    const response = await fetch('./content/trip.json?v=29');
