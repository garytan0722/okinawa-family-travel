import { getVariantDay, mapUrl, nextFixedEvent } from './src/trip-domain.mjs?v=27';
import { centeredScrollLeft, renderChecklistView, renderDay, renderEmergencyView, renderFlightSummary, renderPrivateBookingVault, renderRainyDayView, renderSources, renderStayAction, renderTicketPassGuide, renderVariantTabs, escapeHtml } from './src/render.mjs?v=27';
import { parseOtsReservations } from './src/private-bookings.mjs?v=27';
import { clearRainBackup, selectRainBackup, toggleRainMode } from './src/rain-state.mjs?v=27';
import { createEmptyState, exportBackup, importBackup, loadState, saveState } from './src/storage.mjs?v=27';
import { installPwaUpdate } from './src/pwa-update.mjs?v=27';
...
function renderLogistics() {
  ...
  app.innerHTML = `<section class="tool-view"><p class="eyebrow">FIXED LOGISTICS</p><h1>固定行程與住宿</h1><p class="lede">這些是不能移動的時間；其餘景點都應讓位給它們。</p>${renderFlightSummary(trip)}${renderTicketPassGuide(trip, state.selectedVariant)}<div class="tool-grid">${stays}</div>${renderPrivateBookingVault(state.privateBookings, state.privateStayLocation)}<ol class="fixed-list">${events.map((event) => `<li><time>${dateLabel(event.date)}<strong>${event.time}${event.end ? `–${event.end}` : ''}</strong></time><div><h3>${escapeHtml(event.title)}</h3><p>${escapeHtml(event.place)}</p>${event.note ? `<small>${escapeHtml(event.note)}</small>` : ''}</div></li>`).join('')}</ol></section>`;
}
...
function renderRecords() {
  ...
  app.innerHTML = `<section class="tool-view"><p class="eyebrow">YOUR FIELD NOTES</p><h1>記錄與備份</h1><p class="lede">記錄只保存在這台裝置的瀏覽器；換手機前請先匯出。完整備份可能包含私人地址、確認碼與專屬連結，請勿上傳或轉傳。</p><div class="record-summary"><div><strong>${completed}</strong><span>完成行程</span></div><div><strong>${noted}</strong><span>天有筆記</span></div><div><strong>${state.selectedVariant}</strong><span>目前版本</span></div></div><div class="action-stack"><button type="button" data-action="export-backup">下載完整私人備份</button><label class="file-button">從備份還原<input type="file" accept="application/json" data-action="import-backup"></label><button type="button" class="danger-button" data-action="clear-records">清除本機記錄</button></div><section class="pdf-box"><p class="eyebrow">PRINT EDITIONS</p><h2>A・B・C 三版 PDF</h2><div class="pdf-links"><a href="./output/pdf/okinawa-family-trip-A-balanced.pdf?v=27" download>A 親子平衡</a><a href="./output/pdf/okinawa-family-trip-B-active.pdf?v=27" download>B 景點豐富</a><a href="./output/pdf/okinawa-family-trip-C-relaxed.pdf?v=27" download>C 度假放鬆</a></div></section></section>`;
}
...
function renderQuickPanel() {
  ...
  quickPanel.innerHTML = `<article><span>下一個固定行程</span><strong>${dateLabel(next.date)} · ${next.time}</strong><h3>${escapeHtml(next.title)}</h3><p>${escapeHtml(next.place)}</p></article><article><span>公開版提醒</span><p>住宿卡可開啟 Airbnb 房源；精確地址、入住與門鎖資料仍請從訂房訊息開啟。</p></article>`;
}
...
app.addEventListener('click', (event) => {
  ...
  if (action.dataset.action === 'set-energy') {
    ...
    persist(); renderHome();
  } else if (action.dataset.action === 'toggle-rain-day') {
    toggleRainMode(variantState, action.dataset.rainDate);
    persist(); renderHome();
  } else if (action.dataset.action === 'select-rain-backup') {
    selectRainBackup(variantState, action.dataset.eventId, action.dataset.rainOptionId);
    persist(); renderHome();
  } else if (action.dataset.action === 'clear-rain-backup') {
    clearRainBackup(variantState, action.dataset.eventId);
    persist(); renderHome();
  } else if (action.dataset.action === 'export-backup') {
    if (!confirm('完整備份可能包含私人地址、確認碼與專屬連結。確定要下載嗎？')) return;
  ...
  } else if (action.dataset.action === 'parse-ots-email') {
    const source = document.querySelector('[data-action="ots-email-source"]');
    const reservations = parseOtsReservations(source?.value ?? '');
    const slotForPickup = (pickupAt) => pickupAt.startsWith('2026-09-24') ? 'rental0924' : pickupAt.startsWith('2026-09-30') ? 'rental0930' : '';
    let saved = 0;
    for (const reservation of reservations) {
      const id = slotForPickup(reservation.pickupAt);
      if (!id) continue;
      state.privateBookings[id] = {
        confirmationCode: reservation.confirmationCode,
        detailUrl: reservation.detailUrl,
        contactUrl: reservation.contactUrl,
      };
      saved += 1;
    }
    if (!saved) { alert('找不到 9/24 或 9/30 的 OTS 預約資料，請確認貼上完整郵件。'); return; }
    persist(); renderLogistics(); alert(`已在這台裝置保存 ${saved} 筆 OTS 預約。`);
  } else if (action.dataset.action === 'toggle-private-secret') {
    const input = action.closest('.private-input-row')?.querySelector('input');
    if (!input) return;
    input.type = input.type === 'password' ? 'text' : 'password';
    action.textContent = input.type === 'password' ? '顯示' : '隱藏';
  } else if (action.dataset.action === 'copy-private-field') {
    const input = action.closest('.private-input-row')?.querySelector('input');
    if (!input?.value) return;
    navigator.clipboard?.writeText(input.value).then(() => {
      action.textContent = '已複製';
      setTimeout(() => { action.textContent = '複製'; }, 1200);
    });
  } else if (action.dataset.action === 'open-private-stay') {
    const input = action.closest('.private-input-row')?.querySelector('input');
    if (input?.value.trim()) window.open(mapUrl(input.value.trim()), '_blank', 'noopener,noreferrer');
  } else if (action.dataset.action === 'clear-records' && confirm('確定清除這台裝置上的行程記錄、行前勾選、私人住宿與預約資料？')) {
...
app.addEventListener('input', (event) => {
  if (event.target.dataset.action === 'day-note') {
    ...
    persist();
  } else if (event.target.dataset.action === 'private-stay-location') {
    state.privateStayLocation = event.target.value.trim();
    persist();
  } else if (event.target.dataset.action === 'private-booking-field') {
    const booking = state.privateBookings[event.target.dataset.privateBookingId];
    if (!booking) return;
    booking[event.target.dataset.privateBookingField] = event.target.value.trim();
    persist();
  }
});
...
async function boot() {
  try {
    const response = await fetch('./content/trip.json?v=27');
