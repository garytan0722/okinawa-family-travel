...
test('browser shell resources and dog-paw asset share release v11', () => {
  ...
  assert.deepEqual([...new Set(revisions)], ['11']);
});
...
test('downloadable PDFs use the current release revision in the app and offline cache', () => {
  ...
  assert.ok(urls.every((url) => url.endsWith('?v=11')));
});
...
test('service worker rotates the cache after privacy-sensitive content changes', () => {
  ...
  assert.match(worker, /okinawa-road-book-v11/);
...
test('service worker only removes old caches owned by this app', async () => {
  ...
  const context = {
    caches: {
      keys: async () => ['okinawa-road-book-v10', 'another-pages-app-v1', 'okinawa-road-book-v11'],
  ...
  await activation;
  assert.deepEqual(deleted, ['okinawa-road-book-v10']);
});
...
test('offline navigation falls back to the revisioned HTML shell', async () => {
  ...
  assert.deepEqual(matched, ['./index.html?v=11']);
});
