...
test('a stalled app exposes a static recovery link that preserves local records', () => {
  ...
  assert.match(html, /href="\.\/recovery\.html\?v=22"/);
  assert.match(recovery, /src="\.\/src\/recovery-page\.mjs\?v=22" type="module"/);
...
test('browser shell resources, rain controls, and dog-paw asset share release v22', () => {
  ...
  assert.deepEqual([...new Set(revisions)], ['22']);
});
...
test('downloadable PDFs use the current release revision in the app and offline cache', () => {
  ...
  assert.ok(urls.every((url) => url.endsWith('?v=22')));
});
...
test('service worker rotates the cache after privacy-sensitive content changes', () => {
  ...
  assert.match(worker, /okinawa-road-book-v22/);
...
test('service worker only removes old caches owned by this app', async () => {
  ...
  const context = {
    caches: {
      keys: async () => ['okinawa-road-book-v14', 'another-pages-app-v1', 'okinawa-road-book-v22'],
...
test('offline navigation falls back to the revisioned HTML shell', async () => {
  ...
  assert.deepEqual(matched, ['./index.html?v=22']);
