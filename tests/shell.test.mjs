...
test('browser shell resources and dog-paw asset share release v14', () => {
  ...
  assert.deepEqual([...new Set(revisions)], ['14']);
});
...
test('downloadable PDFs use the current release revision in the app and offline cache', () => {
  ...
  assert.ok(urls.every((url) => url.endsWith('?v=14')));
});
...
test('service worker rotates the cache after privacy-sensitive content changes', () => {
  ...
  assert.match(worker, /okinawa-road-book-v14/);
...
test('service worker only removes old caches owned by this app', async () => {
  ...
  const context = {
    caches: {
      keys: async () => ['okinawa-road-book-v13', 'another-pages-app-v1', 'okinawa-road-book-v14'],
  ...
  await activation;
  assert.deepEqual(deleted, ['okinawa-road-book-v13']);
});
...
test('offline navigation falls back to the revisioned HTML shell', async () => {
  ...
  assert.deepEqual(matched, ['./index.html?v=14']);
